  ...
  <div class="article-item">
    <van-cell>
      ...
      <template #default v-if="+article.cover.type === 1">
        <van-image :src="article.cover.images[0]" fit="cover" class="right-cover"></van-image>
      </template>
      <template #label>
        <div class="cover-wrap">
          <div v-if="+article.cover.type === 3" class="cover-item">
            <van-image v-for="(img, index) in article.cover.images" :key="index" :src="src" fit="cover" class="cover-item-img" ></van-image>
          </div>
